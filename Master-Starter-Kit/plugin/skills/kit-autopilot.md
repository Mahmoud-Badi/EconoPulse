...
The autopilot script lives at `${CLAUDE_PLUGIN_ROOT}/tools/scripts/kit-autopilot.sh`.
Use this path when running it.

If the script doesn't exist at that path, check for it in the project root
(`kit-autopilot.sh`) or create it from the Master Kit template.
...
```bash
SCRIPT=""
if [ -f "${CLAUDE_PLUGIN_ROOT}/tools/scripts/kit-autopilot.sh" ]; then
  SCRIPT="${CLAUDE_PLUGIN_ROOT}/tools/scripts/kit-autopilot.sh"
